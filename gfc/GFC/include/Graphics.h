...
private:
	...
	{
		TTF_Font *pFont;
		int s, h, w, a, d, l;				// font metrics (size, height, width, ascent, descent, line skip)
	};
...
public:
	...
	CGraphics *GetTextGraphics(std::string pText);				// low level text function

	std::string GetFontName()									{ return m_curFontFile; }
	int GetFontSize()											{ return m_curFont.s; }
...
private:
	...
	void SetFont(std::string file, int nPtSize = 0);			// *g << font(file, nPtSize); 
	...
	int  GetFontLeading()						{ return m_curFont.l; }									// n/a
	void SetFontLeading(int l)					{ m_curFont.h += -m_curFont.l + l; m_curFont.l = l; }	// *g << leading(l);
	...
	void GotoRowTp(float r = 0)			{ m_y = GetHeight() - m_mu - (int)((r + 1) * GetFontLeading()); }			// *g << top << row(r);
	void GotoRowBt(float r = 0)			{ m_y = m_mb + (int)(r * GetFontLeading()); }								// *g << bottom << row(r);
	void GotoRowCt(float r = 0)			{ m_y = m_mb + (GetHeight() - m_mu - m_mb - GetFontLeading()) / 2 - (int)(r * GetFontLeading()); }	// *g << vcenter << row(r);
	...
	void GotoColLt(float c = 0)			{ m_x = m_ml + (int)(c * m_curFont.w);  }									// *g << left << col(c);
	void GotoColRt(float c = 0)			{ m_x = GetWidth() - m_mr - (int)(c * m_curFont.w);  }						// *g << right << col(c);
	void GotoColCt(float c = 0)			{ m_x = m_ml + (GetWidth() - m_ml - m_mr) / 2 + (int)(c * m_curFont.w);  }	// *g << center << col(c);
	void GotoLn()						{ m_y -= m_curFont.h * ((m_ss.flags() & std::ios::skipws) ? 1 : -1); GotoCol(); }// *g << endl;
	void GotoLnDn()						{ m_y -= m_curFont.h; GotoCol(); }											// *g << down << endl;
	void GotoLnUp()						{ m_y += m_curFont.h; GotoCol(); }											// *g << up << endl;
};
...
// DEPRECATED FEATURE - use CGraphics Rectangular Fragment Constructors
class EXT_DECL CGraphicsFragment : public CGraphics
{
public:
	CGraphicsFragment(CGraphics *p, int x, int y, int width, int height) : CGraphics(p, CRectangle(x, y, width, height)) { }
	CGraphicsFragment(std::string sFileName, int x, int y, int width, int height) : CGraphics(sFileName, CRectangle(x, y, width, height)) { }
};

// DEPRECATED FEATURE - use CGraphics Tile Constructors
class EXT_DECL CGraphicsTile : public CGraphics
{
public:
	CGraphicsTile(CGraphics *p, int numCols, int numRows, int iCol, int iRow) : CGraphics(p, numCols, numRows, iCol, iRow) { }
	CGraphicsTile(std::string sFileName, int numCols, int numRows, int iCol, int iRow) : CGraphics(sFileName, numCols, numRows, iCol, iRow) { }
};
